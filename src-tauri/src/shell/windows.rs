...
fn get_vscode(mut shells: Vec<SystemShell>) -> Vec<SystemShell> {
    if let Ok(program_files) = env::var("ProgramFiles") {
        ...
        if let Ok(dirs) = fs::read_dir(path) {
            for dir in dirs.filter_map(|x| x.ok()).map(|x| x.path()) {
                let version = dir.file_name().unwrap().to_str().unwrap();
                let bat = dir
                    .join("Community\\Common7\\Tools\\VsDevCmd.bat")
                    .display()
                    .to_string();
...
fn get_wsl(mut shells: Vec<SystemShell>) -> Vec<SystemShell> {
        ...
        .map(|x| x.unwrap())
    {
        let distribution_name: String = RegKey::predef(HKEY_CURRENT_USER)
            .open_subkey(format!("{}\\{}", lxss_path, key))
            .unwrap()
        ...
        shells.push(SystemShell {
            ...
            cwd: None,
            icon: WSL_ICON_MAPPINGS
                .get(distribution_name.clone().as_ref())
                .cloned()
                .unwrap_or("/assets/icons/linux.svg")
                .to_string(),
        })
    }
